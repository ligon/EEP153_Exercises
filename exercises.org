...
!gpg -d --batch --passphrase "PASSPHRASE" FILENAME.json.gpg > FILENAME.json
#+end_src
...
#+end_src
   ...
   In the cell above, we are introduced to a new function that calls a
...
#+end_src
   ...
   The last line we'll run through in depthly is the next one. Similar
...
#+end_src
   ...
   =Python= and =matplotlib=. Check out the docstrings using SHIFT +
   TAB or ?.
...
#+end_src
   ...
   Our final observations: =c= is a string that is modifying =plt= through
   =.title=. We can assume then that =c= is a title of a plot that
   will inevitably be returned as shown by the last line
   =plt.show()=. We also see =df= has a column =t= that
   corresponds to some =k=. Upon closer inspection of =.annotate=,
   this is a function that takes strings and plots it on specific
   points on a map. We can guess that =t= is filled with text labels
   of some sort.
...
f(df,'United Kingdom','Capitals of the countries within the United
Kingdom')
#+end_src
...
def twentyfirst_cent_pop(cntry_code, graph=False):
    ...
    try:
        ...
        if wb.search_countries(cntry_code, display=False) == []:
            print("Hm, couldn't find anything for:", cntry_code)
            return
        else:
            print("Do you mean one of these?")
            results = wb.search_countries(cntry_code, display=False)
            for i in results:
                print(i['id'], "-", i['name'])
            return
...
#+end_src
   ...
   When we pass an empty string or a string full of whitespaces, our
   function raises a TypeError and mentions something about a
   MultiIndex. Using the traceback, we see that line 18 is having an
   issue. We know it successfully gets through the =try= statement, 
   and we can verify that by adding a print statement.
...
** Test Your Understanding
   1. Using =Pandas= and =gspread-dataframe=, each team should submit
      ...
      to jnuesca@berkeley.edu and ligon@berkeley.edu.
...
#+begin_src ipython
# Fix the NameError
pd.DataFrame(data={'Growth Rate':np.diff(chn['Total Population'][::-1])}).plot.line();
plt.title('Population Growth Rate of China from 1960-present');pd.DataFrame(data=np.diff(chn['Total Population'][::-1])).plot.line()
